...
const BuyForm = ({ data, onPurchase }) => {
    ...
    useEffect(() => { setExchange({ ...exchange, converted: Number(exchange.amount / rate).toFixed(8) }) }, [name])

    useEffect(() => { onPurchase(transactions) }, [transactions])
    ...
    const handleChange = ({ target: { value, name } }) => {
        ...
        setExchange({ [name]: value, rate: rate, converted: converted })

    }

    const makePurchase = useCallback((event) => {

        event.preventDefault()

        if (!exchange.amount) { alert("Valor inválido") }
        ...
        <div>
            <form onSubmit={makePurchase}>
                ...
                <div>
                    ...
                    <InputBase label={name} value={exchange.converted} disabled />
